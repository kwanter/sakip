...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('audit_logs', function (Blueprint $table) {
            ...
            $table->enum('compliance_status', ['compliant', 'violation', 'warning'])->nullable();
            $table->text('compliance_notes')->nullable();
            
            // Add performance impact tracking
            $table->enum('impact_level', ['low', 'medium', 'high', 'critical'])->nullable();
        ...
        Schema::create('sakip_audit_trails', function (Blueprint $table) {
            $table->id();
            $table->foreignUuid('audit_log_id')->constrained('audit_logs')->onDelete('cascade');
            $table->string('sakip_module', 50); // performance_data, assessment, report, etc.
            $table->integer('record_id');
            $table->string('record_type', 100); // Model class name
            $table->enum('action_category', ['data_entry', 'assessment', 'approval', 'report_generation', 'system_config']);
    ...
    public function down(): void
    {
        ...
        Schema::table('audit_logs', function (Blueprint $table) {
            ...
            $table->dropColumn(['module', 'compliance_status', 'compliance_notes', 'impact_level']);
        });
    }
};