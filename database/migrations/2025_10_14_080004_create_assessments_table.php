...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('assessments', function (Blueprint $table) {
            $table->id();
            $table->foreignId('performance_data_id')->constrained('performance_data')->onDelete('cascade');
            $table->foreignUuid('assessed_by')->constrained('users')->onDelete('cascade');
            $table->decimal('overall_score', 5, 2)->nullable();
            $table->text('comments')->nullable();
            $table->text('recommendations')->nullable();
            $table->enum('status', ['pending', 'in_review', 'completed', 'approved'])->default('pending');
