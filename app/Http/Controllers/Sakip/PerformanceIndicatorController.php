...
class PerformanceIndicatorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            return view('sakip.indicators.index', compact(
                'indicators',
                'statistics'
            ));

        } catch (\Exception $e) {
            \Log::error('Performance indicators index error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat daftar indikator kinerja.');
        }
    }
    ...
    public function create()
    {
        ...
        try {
            ...
            return view('sakip.indicators.create', compact(
                'instansi',
                'categories',
                'frequencies',
                'collectionMethods'
            ...
            \Log::error('Performance indicator create form error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat formulir pembuatan indikator.');
        }
    }
    ...
    public function store(StorePerformanceIndicatorRequest $request)
    {
        ...
        try {
            ...
            return redirect()->route('sakip.indicators.show', $indicator)
                ->with('success', 'Indikator kinerja berhasil dibuat.');

        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Store performance indicator error: ' . $e->getMessage());
            return back()->withInput()->with('error', 'Terjadi kesalahan saat menyimpan indikator kinerja.');
        }
    }
    ...
    public function show(PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            $currentTargets = $indicator->targets()->where('year', $currentYear)->get();

            // Get recent performance data
            $recentData = $indicator->performanceData()->orderBy('period', 'desc')->limit(6)->get();

            // Calculate performance trends
            $performanceTrend = $this->calculatePerformanceTrend($indicator);

            return view('sakip.indicators.show', compact(
                'indicator',
                'currentTargets',
                'recentData',
                'performanceTrend',
            ...
            \Log::error('Show performance indicator error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat detail indikator.');
        }
    }
    ...
    public function edit(PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            return view('sakip.indicators.edit', compact(
                'indicator',
                'instansi',
                'categories',
                'frequencies',
            ...
            \Log::error('Edit performance indicator form error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat formulir edit.');
        }
    }
    ...
    public function update(UpdatePerformanceIndicatorRequest $request, PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            DB::rollBack();
            \Log::error('Update performance indicator error: ' . $e->getMessage());
            return back()->withInput()->with('error', 'Terjadi kesalahan saat memperbarui indikator kinerja.');
        }
    }
    ...
    public function destroy(PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            if ($indicator->targets()->exists() || $indicator->performanceData()->exists()) {
                return back()->with('error', 'Indikator tidak dapat dihapus karena memiliki data terkait.');
            }
            ...
            DB::rollBack();
            \Log::error('Delete performance indicator error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat menghapus indikator kinerja.');
        }
    }
    ...
    public function getByInstansi(Request $request, Instansi $instansi)
    {
        ...
        try {
            $indicators = PerformanceIndicator::where('instansi_id', $instansi->id)
            ...
            \Log::error('Get indicators by institution error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
     ...
     */
    public function getPerformanceData(Request $request, PerformanceIndicator $indicator)
    {
        $this->authorize('view', $indicator);
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            DB::rollBack();
            \Log::error('Import performance indicators error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
        ...
        try {
            ...
            foreach ($indicators as $indicator) {
                $csv .= implode(',', [
                    ...
                    '"' . str_replace('"', '""', $indicator->calculation_formula) . '"',
                    $indicator->frequency,
                    $indicator->category,
                    $indicator->weight,
            ...
            \Log::error('Export performance indicators error: ' . $e->getMessage());
    ...
    {
        return [
            'total' => PerformanceIndicator::where('instansi_id', $instansiId)->count(),
            'mandatory' => PerformanceIndicator::where('instansi_id', $instansiId)->where('is_mandatory', true)->count(),
            'with_targets' => PerformanceIndicator::where('instansi_id', $instansiId)->has('targets')->count(),
            'with_data' => PerformanceIndicator::where('instansi_id', $instansiId)->has('performanceData')->count(),
            'by_category' => PerformanceIndicator::where('instansi_id', $instansiId)
                ->select('category', DB::raw('count(*) as count'))
                ->groupBy('category')
                ->pluck('count', 'category'),
            'by_frequency' => PerformanceIndicator::where('instansi_id', $instansiId)
    ...
    private function calculatePerformanceTrend(PerformanceIndicator $indicator)
    {
        ...
        $lastYearPerformance = $indicator->performanceData()
