...
class StorePerformanceIndicatorRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'code' => 'required|string|max:50|unique:performance_indicators,code',
    ...
    public function messages(): array
    {
        return [
            ...
            'collection_method.in' => 'The collection method must be one of: manual, automated, survey, interview, observation, document_review.',
            ...
            'category.in' => 'The category must be one of: input, output, outcome, impact.',
            ...
            // Target messages
            'targets.required' => 'Mohon tambahkan minimal satu target tahunan.',
            ...
            'targets.*.year.integer' => 'Tahun harus berupa angka tahun yang valid.',
            'targets.*.year.min' => 'Tahun harus lebih besar atau sama dengan tahun saat ini.',
            'targets.*.target_value.required' => 'Nilai target wajib diisi.',
            'targets.*.target_value.numeric' => 'Nilai target harus berupa angka.',
            'targets.*.target_value.min' => 'Nilai target harus minimal 0.',
            'targets.*.minimum_value.numeric' => 'Nilai minimum harus berupa angka.',
            'targets.*.minimum_value.min' => 'Nilai minimum harus minimal 0.',
            'targets.*.justification.max' => 'Justifikasi tidak boleh lebih dari 500 karakter.',
        ];
    }
}
