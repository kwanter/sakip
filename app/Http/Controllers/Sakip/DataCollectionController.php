...
class DataCollectionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            if ($request->filled('status')) {
                ...
                if ($status === 'missing') {
                    $query->whereDoesntHave('performanceData', function($q) use ($currentYear) {
                        $q->whereYear('period', $currentYear);
                    });
                } elseif ($status === 'completed') {
                    $query->has('performanceData');
                } elseif ($status === 'pending') {
                    $query->whereHas('performanceData', function($q) use ($currentYear) {
                        $q->whereYear('period', $currentYear)
            ...
            return view('sakip.data-collection.index', compact(
                'indicators',
                'statistics',
                'recentEntries',
                'currentYear'
            ...
            return back()->with('error', 'Terjadi kesalahan saat memuat halaman pengumpulan data.');
        }
    }
    ...
    public function create(Request $request, PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            $existingData = $indicator->performanceData()
                ->whereYear('period', $currentYear)
                ->orderBy('period')
                ->get();

            // Get available periods based on frequency
            $availablePeriods = $this->getAvailablePeriods($indicator->frequency, $currentYear);
            ...
            \Log::error('Data collection create form error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat formulir pengumpulan data.');
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $existingData = PerformanceData::where('indicator_id', $indicator->id)
            ...
            $targetValue = $request->get('target_value') ?: ($target ? $target->target_value : null);
            ...
            if ($request->hasFile('evidence_files')) {
                $this->handleEvidenceFiles($request->file('evidence_files'), $performanceData);
            }

            // Validate data quality
            $validationResult = $this->validationService->validatePerformanceData($performanceData);
            ...
            AuditLog::create([
                ...
                'action' => 'CREATE',
                'module' => 'SAKIP',
                'description' => "Memasukkan data kinerja untuk indikator: {$indicator->name} (Periode: {$period->format('M Y')})",
                'old_values' => null,
    ...
    public function edit(PerformanceData $performanceData)
    {
        ...
        try {
            ...
            return view('sakip.data-collection.edit', compact(
                'performanceData',
                'indicator',
                'targets',
                'currentYear'
    ...
    public function update(Request $request, PerformanceData $performanceData)
    {
        ...
        try {
            ...
            // Get target value
            $targetValue = $request->get('target_value') ?: $performanceData->target_value;
            ...
            if ($request->has('existing_files')) {
                ...
                $filesToDelete = $performanceData->evidenceDocuments()
        ...
        try {
            ...
            foreach ($lines as $index => $line) {
                ...
                try {
                    ...
                    $indicator = PerformanceIndicator::where('instansi_id', $user->instansi_id)
                    ...
                    $performancePercentage = $this->calculatePerformancePercentage($actualValue, $targetValue, $indicator->calculation_formula);
            ...
            AuditLog::create([
                ...
                'old_values' => null,
                'new_values' => ['imported_count' => $importedCount, 'year' => $year],
            ]);
            ...
            DB::rollBack();
            \Log::error('Bulk import performance data error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
     ...
     * Validate data quality
     */
    public function validateData(Request $request, PerformanceData $performanceData)
    {
    ...
    private function getDataCollectionStatistics($instansiId, $year)
    {
        ...
        $totalDataEntries = PerformanceData::whereHas('indicator', function($q) use ($instansiId) {
            $q->where('instansi_id', $instansiId);
        })->whereYear('period', $year)->count();

        $pendingReviews = PerformanceData::whereHas('indicator', function($q) use ($instansiId) {
            $q->where('instansi_id', $instansiId);
        ...
        return [
            ...
            'completion_rate' => $totalIndicators > 0 ? round(($indicatorsWithData / $totalIndicators) * 100, 2) : 0,
     ...
     */
    private function handleEvidenceFiles($files, PerformanceData $performanceData)
    {
        foreach ($files as $file) {
            if ($file->isValid()) {
                $filename = time() . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
                $path = $file->storeAs('evidence_documents', $filename, 'public');
     ...
     * Calculate performance percentage
     */
    private function calculatePerformancePercentage($actualValue, $targetValue, $calculationFormula)
    {
     ...
     */
    private function getTargetForPeriod(PerformanceIndicator $indicator, Carbon $period)
    {
     ...
     */
    private function isValidPeriod(PerformanceIndicator $indicator, Carbon $period)
    {
        switch ($indicator->frequency) {
            case 'monthly':
                return $period->day === 1; // Must be first day of month
            case 'quarterly':
                return $period->day === 1 && in_array($period->month, [1, 4, 7, 10]); // First day of quarter
            case 'semester':
                return $period->day === 1 && in_array($period->month, [1, 7]); // First day of semester
            case 'annual':
                return $period->format('Y-m-d') === $period->copy()->startOfYear()->format('Y-m-d'); // First day of year
            default:
                return true;
        }
    }
