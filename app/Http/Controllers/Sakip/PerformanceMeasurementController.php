...
class PerformanceMeasurementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $query = PerformanceIndicator::where('instansi_id', $instansiId)
                ->with(['performanceData' => function($q) use ($year, $quarter) {
            ...
            $metrics = $this->calculateOverallMetrics($instansiId, $year, $quarter, $category);

            // Get performance trends
            $trends = $this->getPerformanceTrends($instansiId, $year, $category);

            // Get benchmark comparisons
            $benchmarks = $this->getBenchmarkComparisons($instansiId, $year, $category);

            return view('sakip.performance-measurement.index', compact(
                'indicators',
                'metrics',
                'trends',
                'benchmarks',
            ...
            \Log::error('Performance measurement index error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat halaman pengukuran kinerja.');
        }
    }
    ...
    public function show(PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            $metrics = $this->calculateIndicatorMetrics($indicator, $currentYear);
            ...
            // Calculate scoring
            $scoring = $this->calculatePerformanceScoring($indicator, $currentYear);

            return view('sakip.performance-measurement.show', compact(
                'indicator',
                'performanceData',
                'targets',
                'metrics',
            ...
            \Log::error('Performance measurement show error: ' . $e->getMessage());
            return back()->with('error', 'Terjadi kesalahan saat memuat detail pengukuran kinerja.');
        }
    }
    ...
    public function calculate(Request $request, PerformanceIndicator $indicator)
    {
        ...
        try {
            ...
            $query = PerformanceData::where('indicator_id', $indicator->id)
            ...
            foreach ($performanceData as $data) {
                $data->update([
                    'performance_percentage' => $calculationResult['individual_scores'][$data->id] ?? null,
    ...
    public function generateReport(Request $request)
    {
        ...
        try {
            ...
            $indicators = PerformanceIndicator::where('instansi_id', $user->instansi_id)
                ->with(['performanceData' => function($q) use ($request) {
            ...
            AuditLog::create([
                ...
                'old_values' => null,
                'new_values' => ['report_type' => $reportType, 'format' => $format, 'year' => $year],
            ]);
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'download_url' => route('sakip.performance-measurement.download-report', ['file' => basename($reportFile)]),
                ],
            ]);
     ...
     * Calculate overall performance metrics
     */
    private function calculateOverallMetrics($instansiId, $year, $quarter = null, $category = null)
    {
        $query = PerformanceData::whereHas('indicator', function($q) use ($instansiId, $category) {
        ...
        $achievedIndicators = $performanceData->where('performance_percentage', '>=', 100)->count();
        $achievementRate = ($totalIndicators > 0) ? ($achievedIndicators / $totalIndicators) * 100 : 0;
    ...
    private function getPerformanceTrends($instansiId, $year, $category = null)
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            $trends[] = $monthData->isNotEmpty() ? round($monthData->avg('performance_percentage'), 2) : 0;
        }
     ...
     * Get benchmark comparisons
     */
    private function getBenchmarkComparisons($instansiId, $year, $category = null)
    {
        // Get institution performance
        $institutionQuery = PerformanceData::whereHas('indicator', function($q) use ($instansiId, $category) {
            $q->where('instansi_id', $instansiId);
            if ($category) {
                $q->where('category', $category);
            }
        ...
        $institutionPerformance = round($institutionQuery->avg('performance_percentage'), 2);

        // Get regional average (all institutions in the same region)
        $regionalQuery = PerformanceData::whereHas('indicator.instansi', function($q) use ($instansiId, $category) {
            $q->where('region_id', function($subQuery) use ($instansiId) {
                $subQuery->select('region_id')->from('instansis')->where('id', $instansiId);
            });
            if ($category) {
                $q->where('category', $category);
            }
        ...
        $regionalPerformance = round($regionalQuery->avg('performance_percentage'), 2);

        // Get national average (all institutions)
        $nationalQuery = PerformanceData::whereHas('indicator', function($q) use ($category) {
            if ($category) {
                $q->where('category', $category);
            }
        })->whereYear('period', $year);

        $nationalPerformance = round($nationalQuery->avg('performance_percentage'), 2);
     ...
     */
    private function calculateIndicatorMetrics(PerformanceIndicator $indicator, $year)
    {
        $performanceData = PerformanceData::where('indicator_id', $indicator->id)
            ->whereYear('period', $year)
            ->get();
        ...
        $achievementRate = ($performanceData->where('performance_percentage', '>=', 100)->count() / $performanceData->count()) * 100;
     ...
     */
    private function getHistoricalPerformance(PerformanceIndicator $indicator, $years)
    {
        $currentYear = Carbon::now()->year;
        $historicalData = [];
        ...
        for ($i = 0; $i < $years; $i++) {
            $year = $currentYear - $i;
            $yearData = PerformanceData::where('indicator_id', $indicator->id)
                ->whereYear('period', $year)
                ->get();
            ...
            if ($yearData->isNotEmpty()) {
                $historicalData[] = [
                    'year' => $year,
                    'average_performance' => round($yearData->avg('performance_percentage'), 2),
    ...
    private function getIndicatorBenchmarks(PerformanceIndicator $indicator)
    {
        ...
        $sectorBenchmarks = $this->benchmarkingService->getSectorBenchmarks($indicator);
        
        // Get regional benchmarks
        $regionalBenchmarks = $this->benchmarkingService->getRegionalBenchmarks($indicator);
        
        // Get national benchmarks
        $nationalBenchmarks = $this->benchmarkingService->getNationalBenchmarks($indicator);
     ...
     */
    private function calculatePerformanceScoring(PerformanceIndicator $indicator, $year)
    {
        $performanceData = PerformanceData::where('indicator_id', $indicator->id)
            ->whereYear('period', $year)
            ->get();
        ...
        $achievementRate = $performanceData->where('performance_percentage', '>=', 100)->count() / $performanceData->count();
        $achievementScore = $achievementRate * 40;

        // Consistency score (0-30 points)
        $standardDeviation = $this->calculateStandardDeviation($performanceData->pluck('performance_percentage'));
        ...
        $overallScore = $achievementScore + $consistencyScore + $improvementScore;
