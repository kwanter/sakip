...
class PerformanceIndicatorService
{
    ...
    public function createIndicator(array $data): PerformanceIndicator
    {
        return DB::transaction(function () use ($data) {
            ...
            $indicator = PerformanceIndicator::create([
                ...
                'code' => $this->generateIndicatorCode($data['instansi_id'], $data['category']),
                'name' => $data['name'],
            ...
            if (isset($data['target_value']) && isset($data['target_year'])) {
                $this->createTarget($indicator, $data['target_year'], $data['target_value']);
            }

            // Log activity
            $this->logActivity('create', $indicator, 'Performance indicator created');
     ...
     */
    public function updateIndicator(PerformanceIndicator $indicator, array $data): PerformanceIndicator
    {
        return DB::transaction(function () use ($indicator, $data) {
            ...
            $affectsHistoricalData = $this->changesAffectHistoricalData($indicator, $data);
            ...
            $indicator->update([
                ...
                'calculation_formula' => $data['calculation_formula'] ?? $indicator->calculation_formula,
            ...
            // Log activity
            $this->logActivity('update', $indicator, 'Performance indicator updated');
    ...
    public function deleteIndicator(PerformanceIndicator $indicator): bool
    {
        return DB::transaction(function () use ($indicator) {
            ...
            if ($indicator->performanceData()->exists()) {
                throw new Exception('Cannot delete indicator with existing performance data');
            }

            // Log activity before deletion
            $this->logActivity('delete', $indicator, 'Performance indicator deleted');
    ...
    {
        return Cache::remember("indicator_{$id}", $this->cacheTimeout, function () use ($id) {
     ...
     */
    protected function validateIndicatorData(array $data, $excludeId = null): \Illuminate\Contracts\Validation\Validator
    {
        $rules = [
        ...
        if ($excludeId) {
            $rules['code'] = 'nullable|string|max:50|unique:performance_indicators,code,' . $excludeId;
        } else {
            $rules['code'] = 'nullable|string|max:50|unique:performance_indicators,code';
        }
    ...
    protected function generateIndicatorCode($instansiId, $category): string
    {
        ...
        $sequence = $lastIndicator ? intval(substr($lastIndicator->code, -4)) + 1 : 1;
     ...
     */
    protected function createTarget(PerformanceIndicator $indicator, $year, $targetValue): Target
    {
     ...
     * Check if changes affect historical data
     */
    protected function changesAffectHistoricalData(PerformanceIndicator $indicator, array $data): bool
    {
        $criticalFields = ['measurement_unit', 'calculation_formula', 'data_source'];
     ...
     */
    protected function handleHistoricalDataChanges(PerformanceIndicator $indicator, array $data): void
    {
        // Log the change for audit purposes
        $this->logActivity('historical_change', $indicator, 'Critical indicator properties changed - historical data may be affected');
     ...
     */
    public function getIndicatorStatistics($instansiId = null, $year = null): array
    {
        $year = $year ?? date('Y');
        ...
        return Cache::remember($cacheKey, $this->cacheTimeout, function () use ($instansiId, $year) {
            ...
            $mandatoryIndicators = (clone $query)->where('is_mandatory', true)->count();
            $indicatorsWithTargets = (clone $query)->whereHas('targets', function ($q) use ($year) {
                $q->where('year', $year);
            })->count();
            $indicatorsWithData = (clone $query)->whereHas('performanceData', function ($q) use ($year) {
            ...
            return [
                ...
                'target_coverage' => $totalIndicators > 0 ? round(($indicatorsWithTargets / $totalIndicators) * 100, 2) : 0,
                'data_coverage' => $totalIndicators > 0 ? round(($indicatorsWithData / $totalIndicators) * 100, 2) : 0,
            ];
        });
    }
     ...
     * Log activity
     */
    protected function logActivity(string $action, PerformanceIndicator $indicator, string $description): void
    {
        AuditLog::create([
            'user_id' => auth()->id(),
