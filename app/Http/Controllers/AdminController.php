...
class AdminController extends Controller
{
    ...
    private function backupMySQL($config, $backupPath, $filename)
    {
        ...
        if ($returnCode !== 0) {
            throw new \Exception('Failed to run mysqldump. Ensure mysqldump is installed.');
        }
    ...
    private function backupPostgreSQL($config, $backupPath, $filename)
    {
        ...
        exec($command, $output, $returnCode);
        if ($returnCode !== 0) {
            throw new \Exception('Failed to run pg_dump. Ensure PostgreSQL client is installed.');
        }
    ...
    private function backupSQLServer($config, $backupPath, $filename)
    {
        ...
        exec($command, $output, $returnCode);
        if ($returnCode !== 0) {
            throw new \Exception('Failed to run SQL Server backup. Ensure sqlcmd is installed.');
        }
