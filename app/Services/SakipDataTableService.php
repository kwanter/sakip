...
class SakipDataTableService
{
    ...
    public function processRequest(Request $request, string $type): array
    {
        ...
        if (!method_exists($this, $method)) {
            throw new \InvalidArgumentException("Data table type '{$type}' not supported");
        }
    ...
    /**
     * Process indicator data table
     */
    protected function processIndicatorTable(Request $request): array
    {
        $query = DB::table('indikator_kinerja')
            ->join('instansi', 'indikator_kinerja.instansi_id', '=', 'instansi.id')
            ->select(
                'indikator_kinerja.*',
        ...
        if ($request->has('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('indikator_kinerja.nama', 'like', "%{$search}%")
                  ->orWhere('indikator_kinerja.kode', 'like', "%{$search}%")
                  ->orWhere('instansi.nama', 'like', "%{$search}%");
            });
        }

        if ($request->has('kategori')) {
            $query->where('indikator_kinerja.kategori', $request->input('kategori'));
        }

        if ($request->has('instansi_id')) {
            $query->where('indikator_kinerja.instansi_id', $request->input('instansi_id'));
        }

        if ($request->has('status')) {
            $query->where('indikator_kinerja.status', $request->input('status'));
        }
        ...
        if ($request->has('sort_by')) {
            ...
            $query->orderBy('indikator_kinerja.created_at', 'desc');
        }
        ...
        $formattedData = $data->map(function ($item) {
            return [
                ...
                'realisasi' => $this->sakipService->formatNumber($item->realisasi),
                'capaian' => $this->sakipService->formatPercentage($item->capaian),
    ...
    protected function processProgramTable(Request $request): array
    {
        ...
        if ($request->has('instansi_id')) {
            $query->where('program.instansi_id', $request->input('instansi_id'));
        }
        ...
        $formattedData = $data->map(function ($item) {
            return [
                ...
                'nama' => $item->nama,
                'instansi' => $item->instansi_name,
                'anggaran' => $this->sakipService->formatCurrency($item->anggaran),
                'realisasi_anggaran' => $this->sakipService->formatCurrency($item->realisasi_anggaran),
                'capaian_anggaran' => $this->sakipService->formatPercentage($item->capaian_anggaran),
                'status' => $this->getStatusBadge($item->status),
                'created_at' => $item->created_at,
                'updated_at' => $item->updated_at,
            ];
    ...
    protected function processKegiatanTable(Request $request): array
    {
        ...
        if ($request->has('instansi_id')) {
            $query->where('kegiatan.instansi_id', $request->input('instansi_id'));
        }
    ...
    protected function processLaporanTable(Request $request): array
    {
        $query = DB::table('laporan_kinerja')
            ->join('instansi', 'laporan_kinerja.instansi_id', '=', 'instansi.id')
        ...
        if ($request->has('instansi_id')) {
            $query->where('laporan_kinerja.instansi_id', $request->input('instansi_id'));
        }
        ...
        if ($request->has('periode')) {
            $query->where('laporan_kinerja.periode', $request->input('periode'));
        }
    ...
    protected function getStatusBadge(string $status): string
    {
        ...
        return '<span class="sakip-status-badge ' . $class . '">' . $label . '</span>';
    }
        ...
        $configs = [
            ...
            'reports' => [
                'columns' => [
                    ...
                    ['field' => 'status', 'title' => 'Status', 'sortable' => true],
                    ['field' => 'creator', 'title' => 'Created By', 'sortable' => true],
                ],
                'filters' => [
