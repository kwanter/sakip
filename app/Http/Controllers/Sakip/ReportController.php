...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            return view('sakip.reports.index', compact(
                'reports',
                'statistics',
                'templates',
                'recentReports',
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            $indicators = PerformanceIndicator::where('instansi_id', $user->instansi_id)
                ->with(['performanceData' => function($q) use ($currentYear) {
            ...
            return view('sakip.reports.create', compact(
                'templates',
                'indicators',
                'availablePeriods',
                'currentYear'
            ...
            return back()->with('error', 'Terjadi kesalahan saat memuat formulir pembuatan laporan.');
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $reportContent = $this->generateReportContent(
                ...
                $request->only(['include_assessments', 'include_benchmarks', 'include_recommendations'])
            );
    ...
    public function show(Report $report)
    {
        ...
        try {
            ...
            return view('sakip.reports.show', compact(
                'report',
                'reportData',
                'relatedReports'
            ));
            ...
            return back()->with('error', 'Terjadi kesalahan saat memuat detail laporan.');
        }
    }
    ...
    public function generateFile(Request $request, Report $report)
    {
        ...
        try {
            ...
            return response()->json([
                ...
                'data' => [
                    'file_path' => $filePath,
                    'download_url' => route('sakip.reports.download', ['report' => $report->id]),
                ],
            ]);
    ...
    public function submitForApproval(Request $request, Report $report)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat mengirim laporan untuk persetujuan.',
            ], 500);
        }
    }
    ...
    public function approve(Request $request, Report $report)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat memproses persetujuan laporan.',
            ], 500);
        }
    }
    ...
    private function getReportStatistics($user, $year)
    {
        ...
        $byType = $query->select('report_type', DB::raw('count(*) as count'))
            ->groupBy('report_type')
     ...
     * Generate report content
     */
    private function generateReportContent(Report $report, array $indicatorIds, array $options)
    {
        try {
            $user = Auth::user();
            ...
            $indicators = PerformanceIndicator::whereIn('id', $indicatorIds)
                ...
                ->with(['performanceData' => function($q) use ($year) {
    ...
    private function generateReportFile(Report $report, string $format)
    {
        try {
            ...
            switch ($format) {
                case 'pdf':
                    return $this->reportService->generatePDFFile($report, $reportData);
                case 'excel':
                    return $this->reportService->generateExcelFile($report, $reportData);
                case 'word':
                    return $this->reportService->generateWordFile($report, $reportData);
                default:
    ...
    private function calculateReportSummary(Report $report)
    {
        ...
        $indicatorsWithData = $indicators->filter(function($indicator) {
        ...
        $achievedIndicators = $indicators->filter(function($indicator) {
        ...
        return [
            ...
            'achievement_rate' => $totalIndicators > 0 ? round(($achievedIndicators / $totalIndicators) * 100, 2) : 0,
        ];
    }
    ...
    private function getReportTrends(Report $report)
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            $monthlyData = PerformanceData::whereIn('indicator_id', $report->indicators->pluck('id'))
            ...
            $trends[] = [
                ...
                'average_performance' => $monthlyData->isNotEmpty() ? round($monthlyData->avg('performance_percentage'), 2) : 0,
    ...
    private function getReportBenchmarks(Report $report)
    {
        ...
        $regionalPerformance = $this->calculateRegionalPerformance($instansiId, $year);
    ...
    {
        return PerformanceData::whereHas('indicator', function($q) use ($instansiId) {
    ...
    {
        return PerformanceData::whereHas('indicator.instansi', function($q) use ($instansiId) {
    ...
    {
        return PerformanceData::whereYear('period', $year)
          ->avg('performance_percentage') ?? 0;
    }
    ...
    private function getAvailableReportPeriods()
    {
        ...
        foreach ([1, 7] as $month) {
            $periods[] = [
                'value' => Carbon::create($currentYear, $month, 1)->format('Y-m-d'),
                'label' => ($month === 1 ? 'Semester 1' : 'Semester 2') . ' ' . $currentYear,
            ];
        }
